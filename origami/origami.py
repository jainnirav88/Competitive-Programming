...
class PaneCommand(sublime_plugin.WindowCommand):
    ...
    def zoom_pane(self, fraction, myOwn):
        if fraction is None:
            fraction = .9
        ...
        #changed this 3 lines, remove if any problem
        if myOwn == 1: 
            current_row_height = 1.0 / num_rows
            other_row_height = current_row_height
...
class ZoomPaneCommand(PaneCommand):
    def run(self, fraction=None, myOwn=0):
        self.zoom_pane(fraction, myOwn)
